 ...
 *
 * Orchestrates resident registration via invitation token.
...
import { ResidentRegistrationRequest } from '@domain/models/invitation/invitation.model';
...
export class RegisterResidentUseCase {
  ...
  execute(request: ResidentRegistrationRequest): Observable<Result<void>> {
    return this.invitationGateway.registerResident(request).pipe(
